...
server.tool(
    ...
    "Find documents in a MongoDB collection",
    {
        ...
        query: z.record(z.any()).describe("The query filter").optional().default({}),
        options: z.object({
            ...
            sort: z.record(z.number()).optional().describe("Sort criteria (e.g. {field: 1} for ascending, {field: -1} for descending)"),
            projection: z.record(z.number()).optional().describe("Fields to include or exclude")
        }).optional().default({})
    },
    async (arg) => {
        try {
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: result ? `Found document in collection '${collection}'` : `No document found in collection '${collection}' matching the query`
                    },
                    {
                        type: "text",
...
server.tool(
    ...
    {
        collection: z.string().describe("The collection name"),
        filter: z.record(z.any()).describe("The filter to find the document to update"),
        update: z.record(z.any()).describe("The update operations to apply"),
        options: z.object({
            upsert: z.boolean().optional().describe("Create a document if no documents match the filter")
        }).optional().default({})
    },
    async (arg) => {
        try {
            const { collection, filter, update, options } = arg;
            const result = await mongoService.updateOne(collection, filter, update, options);
...
server.tool(
    ...
    {
        collection: z.string().describe("The collection name"),
        filter: z.record(z.any()).describe("The filter to find documents to update"),
        update: z.record(z.any()).describe("The update operations to apply"),
        options: z.object({
            upsert: z.boolean().optional().describe("Create a document if no documents match the filter")
        }).optional().default({})
    ...
    async (arg) => {
        try {
            const { collection, filter, update, options } = arg;
            const result = await mongoService.updateMany(collection, filter, update, options);
...
server.tool(
    ...
    {
        collection: z.string().describe("The collection name"),
        filter: z.record(z.any()).describe("The filter to find the document to delete")
    },
    async (arg) => {
        try {
...
server.tool(
    ...
    {
        collection: z.string().describe("The collection name"),
        filter: z.record(z.any()).describe("The filter to find documents to delete")
    },
    async (arg) => {
        try {
            const { collection, filter } = arg;
...
server.tool(
    ...
    {
        collection: z.string().describe("The collection name"),
        pipeline: z.array(z.record(z.any())).describe("The aggregation pipeline stages")
    },
    async (arg) => {
        try {
